#perform a MuSSE analysis for WGD events in Monocots

#install.packages("/rhome/jlandis/Rlibs/ape_3.5.tar.gz",repos=NULL,type="source")
#install.packages("/rhome/jlandis/Rlibs/diversitree_0.9-9.tar.gz", repos=NULL, type="source")
library(diversitree)
library(ape)
library(geiger)
library(nlme)
...
mydata <- read.csv("Monocots_MuSSE.csv",header=TRUE, row.names=1)
